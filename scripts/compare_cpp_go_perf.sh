...
printf "[1/4] Running Go benchmarks (GOMAXPROCS=%s)...\n" "${THREADS}"
GOMAXPROCS="${THREADS}" go test ./go-iml/cache -run '^$' -bench 'BenchmarkCache(Set|Get|ConcurrentSetGet)$' -benchmem -count=3 | tee "${GO_OUT}"
...
printf "\n[4/4] Running C++ benchmarks (threads=%s)...\n" "${THREADS}"
"${ROOT_DIR}/cpp-impl/build/benchmark_sharded_cache" --ops 300000 --threads "${THREADS}" | tee "${CPP_OUT}"
